...
FROM dept 
WHERE loc = 'NEW YORK';
-- DUAL 가상테이블이름. 테이블이 없는 내용을 selsct할때
...
,NVL2(comm,100,0), E.* FROM emp E; -- WHERE NVL(comm,0) = 0;
...
-- 문자열을 연결할때 concat함수외에 ||파이프라인 2개를 겹쳐서 구현
SELECT ename ||'is a'|| Job AS "문자열 연결" FROM emp;
-- 여기까지 srlect 마무리 Read
...
-- Instr(문자의 위치를 구하는함수), Substr(매개변수로 입력한 숫자위치만큼 문자열을 추출하는함수)
