...
--ERD로 게시판테이블-[댓글|첨부파일]제약조건(constraint)이 자동생성 Foreign KEY(외래키)부자관계
--14장 트랜잭션 DB단에서 사용하지 않고, 스프링단에서 트랜잭션을 사용
--스프링단에서
--commit;(DML문:insert,update,delete)
--rollback는 제일 마지막 커밋된 상태로 되돌립니다/
--12장테이블 구조생성(create;), 변경(alter), 삭제(drop;)
--ERD관계형 다이어그램으로 물리적 테이블 생성(포워드엔지니어링)
...
--11서브쿼리
--단일행서브쿼리 필드값을 비교할때, 비교할때, 비교하는 값인 단일한
...
SELECT R.deptno, R.dept_sal FROM (
...
SELECT ename, round(sal) FROM emp;--레코드가 여러개
...
UPDATE emp01 SET ename = '구꽁이' WHERE empno = 7839;
