#### 작업예정.
- 스프링 프로젝트 순서
- 1. JUnit > 마이바티스(DB핸들링) > AOP(다중게시판기능) > 페이징기능 > 검색기능 > 첨부파일기능(파일업드로/다운로드) > 스프링시큐리티(로그인 인증/권한체크) > 
- 2. 댓글기능(RestAPI-백엔드, Ajax처리-프런트단) > 네이버아이디로그인(외부API사용) > 헤로쿠클라우드배포
- 3.문서작업(화면기획서XLS 제작, 화면설계PPT 제작)
...
- 스피링에서는 개발자가 처리하는 100의코딩에서 2-3개 정도(파일업로드,다운로드)이고, 대부분throw Exception스프링으로 넘김.(예외의 전파)
...
- 출력하는 방식 : 25년전부터 방식 EL(Express Language)방식출력-${변수} , <%=${변수}%>
...
- 1.@Controller 클래스빈 : 라우터역할 메서드를 만듭니다.(라우터역할의 클래스가능)
- 2.@Service 클래스빈 : 비지니스로직 메서드를 만듭니다.(개발자위주의 외부클래스 기능)
- 3.@Repository 클래스빈 : MOdel처리 메서드를 만듭니다.(DB핸들링외부클래스사용)
- 위 3가지의 @를 사용하지 않는 클래스는 DI(객체생성-의존성주입)x, AOP(x), IoC(제어의역전-자동메모리관리)x
- 프론트개발자가 작업한 결과 백엔드개발자가 확인.
- 일괄바꾸기 1. "/home -> "/resources/home
- 일괄바꾸기 2. '/home -> '/resources/home
- ERD기준으로 게시판UI 마무리합니다.-board_write.html 부터 시작
- VScode 작업한 html를 이클립스에 배치를 합니다.(프론트개발자가 작업결과를 백엔드개발자가 받아서 배치합니다.)
- resources 폴더에 static컨텐츠(html, css, js, font) 배치합니다.
